...
coverage=$(tail -n +2 temp.csv | awk -F',' '{ covered+=$5; missed+=$4 } END { print (covered / (covered + missed)) * 100 }')
covered=$(tail -n +2 temp.csv | awk -F',' '{ covered+=$5 } END { print covered }')
total_instructions=$(tail -n +2 temp.csv | awk -F',' '{total+=$5+$4} END {print total}')
